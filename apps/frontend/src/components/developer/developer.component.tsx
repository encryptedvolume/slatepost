...
const CopyButton = ({
  ...
  return (
    <button
      ...
      className="cursor-pointer px-[16px] h-control bg-btnSimple hover:bg-boxHover transition-colors rounded-control t-control flex items-center gap-[8px]"
    >
      <svg
        width="14"
        height="14"
  ...
  if (!app && !creating) {
    return (
      <div className="flex flex-col gap-[48px]">
        <div className="t-control text-textColor">
          {t(
            'oauth_app_note_line1',
            'Create an OAuth App to let other Slate users authorize your product to post on their behalf.'
          )}
          <br />
          {t(
        ...
        <div className="bg-newBgColorInner rounded-card border border-newBorder overflow-hidden">
          <div className="bg-newBgColorInner px-[16px] py-[16px] border-b border-newBorder flex items-start justify-between gap-[12px]">
            <div>
              ...
              <div className="t-secondary text-inkSecondary mt-[4px]">
                {t(
                  ...
                  'Create an OAuth application to allow third-party integrations with Slate on behalf of your users.'
                )}
              </div>
            </div>
            <div className="flex gap-[8px] shrink-0 pt-[4px]">
              <a
                className="cursor-pointer px-[16px] h-control bg-primaryBg hover:bg-primaryBgHover text-primaryText transition-colors rounded-control t-control flex items-center gap-[8px]"
                href="https://docs.slatepost.lol/public-api/oauth"
                target="_blank"
              >
          ...
          <div className="p-[20px]">
            <button
              ...
              className="cursor-pointer px-[16px] h-large bg-primaryBg hover:bg-primaryBgHover transition-colors text-primaryText rounded-control t-control"
            >
  ...
  if (creating && !app) {
    return (
      <div className="flex flex-col gap-[48px]">
        ...
        <div className="bg-newBgColorInner rounded-card border border-newBorder overflow-hidden">
          <div className="bg-newBgColorInner px-[16px] py-[16px] border-b border-newBorder">
            <div className="t-title-3">
              {t('create_oauth_app', 'Create OAuth App')}
            </div>
            <div className="t-secondary text-inkSecondary mt-[4px]">
          ...
          <div className="p-[20px] flex flex-col gap-[16px]">
            <div className="flex flex-col gap-[8px]">
              <label className="t-control text-inkSecondary">
                {t('app_name', 'App Name')} *
              </label>
              <input
                className="bg-newBgColorInner border border-newBorder rounded-control px-[16px] h-large text-textColor outline-none"
                value={name}
            ...
            <div className="flex flex-col gap-[8px]">
              ...
              <textarea
                className="bg-newBgColorInner border border-newBorder rounded-control p-[16px] text-textColor outline-none min-h-[80px]"
                value={description}
              ...
              <div className="flex items-center gap-[12px]">
                {picturePath ? (
                  <img
                    src={picturePath}
                    alt="App picture"
                    className="w-[48px] h-large rounded-pill object-cover"
                  />
                ) : (
                  <div className="w-[48px] h-large rounded-pill bg-btnSimple flex items-center justify-center text-inkSecondary">
                    ?
                  </div>
                )}
                <button
                  type="button"
                  onClick={openMedia}
                  className="cursor-pointer px-[16px] h-control bg-btnSimple hover:bg-boxHover transition-colors rounded-control t-control"
                >
            ...
            <div className="flex gap-[8px]">
              ...
              <button
                ...
                className="cursor-pointer px-[16px] h-large bg-btnSimple hover:bg-boxHover transition-colors rounded-control t-control"
              >
                {t('cancel', 'Cancel')}
              </button>
            </div>
          ...
          <div className="p-[20px] flex flex-col gap-[16px]">
            <div className="flex items-center gap-[12px]">
              {app.picture?.path ? (
                ...
                <div className="w-[48px] h-large rounded-pill bg-btnSimple flex items-center justify-center text-inkSecondary t-title-3">
                  {app.name?.[0]?.toUpperCase() || '?'}
                </div>
              )}
              <div>
                <div className="t-title-3">{app.name}</div>
                {app.description && (
            ...
            <div className="flex flex-col gap-[4px]">
              ...
              <div className="t-control">{app.redirectUrl}</div>
            </div>
            <div className="flex gap-[8px]">
              <button
                type="button"
      ...
      <div className="bg-newBgColorInner rounded-card border border-newBorder overflow-hidden">
        ...
        <div className="p-[20px] flex flex-col gap-[16px]">
          <div className="flex flex-col gap-[8px]">
            ...
            <div className="bg-newBgColorInner border border-newBorder rounded-control px-[16px] h-large flex items-center overflow-hidden">
              <code className="t-control flex-1 truncate">{app.clientId}</code>
            </div>
          </div>
          <div className="flex flex-col gap-[8px]">
            <div className="t-control text-inkSecondary">
            ...
            <div className="bg-newBgColorInner border border-newBorder rounded-control px-[16px] h-large flex items-center overflow-hidden">
              {plaintextSecret ? (
                <code className="t-control flex-1 truncate">
                  {plaintextSecret}
                </code>
              ) : (
                <span className="text-inkSecondary t-secondary">
                  {t(
