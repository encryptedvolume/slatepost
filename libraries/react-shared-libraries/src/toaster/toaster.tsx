...
export const Toaster = () => {
  ...
  return (
    // Toasts are one of the five places elevation is allowed: overlay surface,
    // 1px hairline, 14 radius, the single overlay shadow. No glow, no gradient.
    <div
      className={clsx(
        'animate-fadeDown rounded-card gap-[12px] flex items-center bg-surfaceOverlay border border-line shadow-overlay',
        'px-[16px] min-w-[319px] fixed start-[50%] text-ink z-[900] top-[32px] -translate-x-[50%] h-[56px]'
      )}
    >
      <div
        className={clsx(
          'w-[20px] h-[20px] flex items-center justify-center [&>svg]:w-[20px] [&>svg]:h-[20px]',
          toasterType === 'success' ? 'text-success' : 'text-critical'
        )}
      >
        {toasterType === 'success' ? (
          <svg
      ...
      <div className="flex-1 t-body-emphasis text-ink">{toasterText}</div>
    </div>
  );
};
